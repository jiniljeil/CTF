...
MIN_PORT = 31000
MAX_PORT = 34000
POW_DIFFICULTY = 7  # Number of leading zeros required in the PoW hash
INSTANCE_PREFIX = "geoweapon_"
SITE = "35.94.90.25"
INSTANCE_LIFETIME_MINUTES = 20
...
def handle_client(client_socket):
    ...
    client_socket.send(f"Solve PoW: hashlib.sha256(b'{challenge}' + <your_input>).hexdigest().startswith('0' * {POW_DIFFICULTY})\nInp:  ".encode())
    
    nonce = client_socket.recv(1024).decode().strip()
    
    if verify_pow(challenge, nonce, POW_DIFFICULTY):
        if len(instances.keys()) >= 20:
            client_socket.send(f"Reached Max Instances :(, try later or contact admin")
        else:
