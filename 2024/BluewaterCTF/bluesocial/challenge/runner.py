...
MIN_PORT = 25000
MAX_PORT = 26000
POW_DIFFICULTY = 4  # Number of leading zeros required in the PoW hash
INSTANCE_PREFIX = "social_"
...
def handle_client(client_socket):
    ...
    client_socket.send(f"Solve PoW: {challenge}\nSolution: ".encode())
    
    nonce = client_socket.recv(1024).decode().strip()
    
    if not verify_pow(challenge, nonce, POW_DIFFICULTY):
